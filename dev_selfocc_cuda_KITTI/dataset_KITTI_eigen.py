...
class KITTI_eigen(data.Dataset):
    def __init__(self, entries, inheight, inwidth, maxinsnum, root, ins_root, depth_root, depthvls_root, mdPred_root=None,
                 flowPred_root=None, RANSACPose_root=None, baninsmap=False, istrain=False, isgarg=False):
    ...
    def __getitem__(self, index):
        ...
        img1, img2, depth, depthvls, mdDepth_pred, inspred, flowpred_RAFT, intrinsic = self.aug_crop(img1, img2, depth, depthvls, mdDepth_pred, inspred, flowpred_RAFT, intrinsic)

        if self.istrain:
            img1, img2 = self.colorjitter(img1, img2)

        data_blob = self.wrapup(img1=img1, img2=img2, depthmap=depth, depthvls=depthvls, mdDepth_pred=mdDepth_pred, intrinsic=intrinsic, insmap=inspred, flowpred=flowpred_RAFT, rel_pose=rel_pose, posepred=posepred, tag=self.entries[index])
        return data_blob
    ...
    def wrapup(self, img1, img2, depthmap, depthvls, mdDepth_pred, intrinsic, insmap, flowpred, rel_pose, posepred, tag):
    ...
    def aug_crop(self, img1, img2, depthmap, depthvls, mdDepth_pred, instancemap, flowpred_RAFT, intrinsic):
        if img1.ndim == 3:
            h, w, _ = img1.shape
        else:
            h, w = img1.shape
        ...
        return img1, img2, depthmap, depthvls, mdDepth_pred, instancemap, flowpred_RAFT, intrinsic

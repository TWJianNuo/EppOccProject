...
from core.eppcore import eppcore_inflation, eppcore_compression
...
class EppNet_Encoder(nn.Module):
    def __init__(self):
        super(EppNet_Encoder, self).__init__()
...
class EppNet_Decoder(nn.Module):
    def __init__(self):
        super(EppNet_Decoder, self).__init__()
...
class EppNet(nn.Module):
    def __init__(self, args):
        ...
        self.encorder = EppNet_Encoder()
        self.decoder = EppNet_Decoder()
    ...
    def get_samplecoords(self, depthpred, intrinsic, posepred, insmap, dsratio, orgh, orgw):
        ...
        projM = projM[:, 0]
        projM = projM.view([bz, 1, 1, 1, 4, 4]).expand([-1, self.nedges, orgh, orgw, -1, -1])
    ...
    def speed_test(self, image1, image2, depthpred, intrinsic, posepred, insmap):
        """ Estimate optical flow between pair of frames """
        _, _, orgh, orgw = image1.shape
        image1_normed = 2 * image1 - 1.0
        image2_normed = 2 * image2 - 1.0

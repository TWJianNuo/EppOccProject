...
class ReconImages(nn.Module):
    """Layer to transform a depth image into a point cloud
    """
    def __init__(self):
        super(ReconImages, self).__init__()
        self.compdict = dict()

    def forward(self, imgr, depthmap, intrinsic, poser):
        bz, _, h, w = depthmap.shape

        key = "{}_{}".format(h, w)

        if key not in self.compdict.keys():
            ...
            self.compdict[key] = torch.stack([xx, yy, ones], dim=3)

        xx, yy, ones = torch.split(self.compdict[key], [1, 1, 1], dim=3)
